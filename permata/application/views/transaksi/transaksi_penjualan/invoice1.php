...
<table class="table table-hover">
	...
	foreach ($result as $lap) { ?>
		<tr class="table-secondary">
			...
			<td align="center">
				<?php
					$min = $this->Transaksi_penjualan_model->pj_minus($lap->no_trx);
					if ($min == 1) {
			...
			<?php if ($this->session->userdata('jabatan') == "Pegawai") { if ($min == 1) { ?>
			<td align="center">
				<a href="<?=base_url('index.php/transaksi_penjualan/editnota/'.$lap->no_trx);?>"><i class="fa fa-pencil-square-o"></i></a>
			</td>
			<?php }else{ ?>
			<td align="center">&nbsp;</td>
			<?php } } else if ($lap->id_user == $this->session->userdata('id')) { if ($min == 1) { ?>
			<td align="center">
				<a href="<?=base_url('index.php/transaksi_penjualan/editnota/'.$lap->no_trx);?>"><i class="fa fa-pencil-square-o"></i></a>
			</td>
			<?php } else { ?>
